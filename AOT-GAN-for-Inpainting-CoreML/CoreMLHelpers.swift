...
extension UIImage {
    func mlMultiArray(scale preprocessScale:Double=1/255,
        ...
        let mlArray = try! MLMultiArray(shape: [1,3,  NSNumber(value: size.height), NSNumber(value: size.width)], dataType: MLMultiArrayDataType.double)
    ...
    func mlMultiArrayGrayScale(scale preprocessScale:Double=1/255,bias preprocessBias:Double=0) -> MLMultiArray {
        ...
        let size = self.size
        let imagePointer : UnsafePointer<Double> = UnsafePointer(imagePixel)
        let mlArray = try! MLMultiArray(shape: [1,1,  NSNumber(value: size.height), NSNumber(value: size.width)], dataType: MLMultiArrayDataType.double)
        mlArray.dataPointer.initializeMemory(as: Double.self, from: imagePointer, count: imagePixel.count)
        return mlArray
    }
